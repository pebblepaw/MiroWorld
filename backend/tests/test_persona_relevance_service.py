...
def test_parse_sampling_instructions_falls_back_when_gemini_returns_only_notes(tmp_path, monkeypatch):
    settings = Settings(simulation_db_path=str(tmp_path / "sim.db"))
    service = PersonaRelevanceService(settings)
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (
            ...
            '"notes_for_ui": ["Younger adults and educators are relevant."]}'
        ),
    )

    parsed = service.parse_sampling_instructions(
        "Prioritise younger adults, young families, and educators affected by education affordability and childcare pressure.",
        knowledge_artifact={"summary": "Birth-rate and childcare pressure in Singapore.", "entity_nodes": []},
    )

    assert parsed["source"] == "fallback"
    assert "youth" in parsed["soft_boosts"]["age_cohort"]
    assert "public_administration_education_services" in parsed["soft_boosts"]["industry"]
